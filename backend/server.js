...
const initializeDataFile = (fileName, defaultData = []) => {
  ...
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(
      filePath,
      `module.exports = ${JSON.stringify(defaultData, null, 2)};`
    );
  }
};
...
app.post("/questionnaires", (req, res) => {
  try {
    ...
    fs.writeFileSync(
      path.join(databasePath, "questionnaires.js"),
      `module.exports = ${JSON.stringify(questionnairesData, null, 2)};`
    );
...
app.delete("/questionnaires/:id", (req, res) => {
  try {
    ...
    console.error("Error en DELETE /questionnaires/:id:", error.message);
    res.status(500).json({ error: error.message });
  }
});
